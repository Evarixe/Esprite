...
def evaluate(model, loader, device, max_batches: int = 20) -> dict:
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            fam   = batch["family_index"].to(device, non_blocking=True)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = model(tokens, x_pos, y_pos, f_pos, roles, attn, id_index=idi, family_index=fam, color_rgb=crgb)
...
def main():
    ...
    # Registres identite/famille -> tailles des tables d'embedding (spec v2).
    reg_path = args.data / "identity_registry.json"
    fam_path = args.data / "family_registry.json"
    n_identities = len(json.loads(reg_path.read_text(encoding="utf-8"))) if reg_path.exists() else 1024
    n_families = len(json.loads(fam_path.read_text(encoding="utf-8"))) if fam_path.exists() else 512
    model = SpriteTransformer(n_identities=n_identities, n_families=n_families).to(device)
    print(f"[train] model params: {model.n_params/1e6:.2f}M | {n_identities} identites | {n_families} lignees")
